...
export default function Seminars() {
  return (
    <section className="py-12">
      <h1 className="text-3xl font-bold mb-8 text-blue-900 text-center">Upcoming Seminars</h1>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {seminars.map((seminar) => (
          <div key={seminar.id} className="bg-white shadow-lg rounded-lg p-6 hover:shadow-xl transition">
            ...
            <div className="text-gray-600 mb-1">{new Date(seminar.date).toLocaleString()}</div>
            <div className="text-gray-700 mb-1">Speaker: {seminar.speaker}</div>
            <div className="text-gray-700 mb-1">Location: {seminar.location}</div>
